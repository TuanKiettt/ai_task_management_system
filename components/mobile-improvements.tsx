...
import { 
  ChevronDown, ChevronRight, Plus, Filter, Search, X,
  User, Calendar, Flag, Circle, CheckCircle2, Loader2, AlertCircle, Edit
} from "lucide-react"
...
const Sheet = ({ children, isOpen, onClose }: { 
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      <div className="relative bg-card border-t border-border rounded-t-xl w-full max-h-[50vh] overflow-y-auto p-4 animate-in slide-in-from-bottom">
        {children}
      </div>
    </div>
  )
...
export function MobileImprovements() {
            ...
            {showTaskActions === task.id && (
              <div className="absolute right-0 top-full mt-1 w-48 bg-card border border-border rounded-lg shadow-lg z-10">
                ...
                <button
                  ...
                  className="w-full text-left px-4 py-2 hover:bg-muted transition-colors flex items-center gap-2 text-green-600"
                >
                  <CheckCircle2 className="w-4 h-4" />
                  Mark Complete
                </button>
