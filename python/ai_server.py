...
class TrueAIServer:
    ...
    def predict_category(self, text: str) -> tuple:
        ...
        try:
            ...
            with torch.no_grad():
                ...
                # Softmax để get probabilities
                predictions = torch.nn.functional.softmax(outputs.logits, dim=-1)
                # Argmax để get predicted class
    ...
    def extract_entities(self, text: str) -> dict:
        ...
        # Map to our expected format with fallback values
        return {
            'time': entities.get('time') or 'No time specified',
            'date': entities.get('date') or None,
            'location': entities.get('location') or 'No location specified',
            'people': entities.get('people', []),
            'subject': entities.get('subject') or 'No subject',
            'action': entities.get('action') or 'No action',
            'organization': entities.get('organization') or None,
            'budget': entities.get('budget') or None,
            'priority_indicator': entities.get('priority_indicator') or None
        }
    ...
    def normalize_date(self, date_str: str) -> str:
        """Normalize date string - handles both numeric and natural language dates"""
        if not date_str:
            return None
        ...
        natural_date = self.extract_natural_date(date_str)
        if natural_date:
            return natural_date
        
        # Try numeric format (DD/MM or DD/MM/YYYY)
        try:
            if '/' in date_str:
    ...
    def extract_natural_date(self, date_str: str) -> str:
        """Extract date from natural language expressions using dateparser"""
        try:
            # Use dateparser for comprehensive natural language date parsing
            # Supports 40+ languages and various formats
            parsed_date = parse(
                date_str,
    ...
    def process_chat_message(self, message: str, user_id: str, context: dict) -> dict:
        ...
        prediction = {
            ...
            "priority": priority,
            "confidence": overall_confidence,
            "deadline": entities.get('date') or 'No deadline specified',
            "entities": entities
        }
        ...
        return {
            ...
            "model_info": {
                ...
                "dataset": 'MS-LaTTE (Microsoft Locations and Times of Task Execution) + OntoNotes 5.0',
    ...
    def process_task_message(self, message: str, user_id: str, context: dict) -> dict:
        ...
        task = {
            ...
            "priority": priority.title(),
            "time": entities.get('time') or 'No time specified',
            "date": due_date or 'No date specified',
            "confidence": overall_confidence,
            "hasDate": bool(due_date),
        ...
        return {
            ...
            "confidence": overall_confidence,
            "model_info": {
                "type": 'Persistent Custom-Trained MS-LaTTE + DistilBERT',
